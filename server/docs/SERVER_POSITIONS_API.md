...
| `z` | нет | высота — сохраняется и копится в карту высот |
...
Высота `z` пишется в позицию игрока и в сетку высот карты
(`GET /api/maps/{slug}/elevation?x=&y=`). ПКМ → «Копировать #Teleport» подставляет Z из этой сетки.
